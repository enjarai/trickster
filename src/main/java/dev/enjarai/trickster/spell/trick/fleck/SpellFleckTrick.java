...
public class SpellFleckTrick extends AbstractFleckTrick<SpellFleckTrick> {
    public SpellFleckTrick() {
        super(
                ...
                Signature.of(FragmentType.NUMBER, FragmentType.VECTOR, FragmentType.VECTOR, FragmentType.SPELL_PART,
                        FragmentType.ENTITY.variadicOfArg().unpack().optionalOfArg(), SpellFleckTrick::run, FragmentType.NUMBER)
        );
    }

    public NumberFragment run(SpellContext ctx, NumberFragment id, VectorFragment position, VectorFragment facing, SpellPart spell, Optional<List<EntityFragment>> targets) {
