...
public class ModModelGenerator extends FabricModelProvider {
    ...
    public void generateBlockStateModels(BlockStateModelGenerator blockStateModelGenerator) {
        blockStateModelGenerator.blockStateCollector.accept(
                VariantsBlockStateSupplier.create(ModBlocks.SPELL_RESONATOR)
                        .coordinate(
                                BlockStateVariantMap.create(SpellResonatorBlock.FACING, SpellResonatorBlock.POWER)
                                        .register(
                                                (direction, i) -> BlockStateVariant.create()
                                                        .put(
                                                                VariantSettings.MODEL,
                                                                i != 0 ? Trickster.id("block/spell_resonator_enabled") : Trickster.id("block/spell_resonator")
                                                        )
                    ...
                    VariantsBlockStateSupplier.create(block)
                            .coordinate(
                                    BlockStateVariantMap.create(Properties.FACING)
                                            .register(
                                                    direction -> BlockStateVariant.create()
